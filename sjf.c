...
/*Sorting According to Arrival Time*/ for(i=0;i<n;i++)
{
for(j=0;j<n;j++)
{
if(at[i]<at[j])
...
//to calculate waiting time wt[0]=0;
for(i=1;i<n;i++)
{
sum=sum+bt[i-1]; 
wt[i]=sum-at[i]; 
...
//to calculate Turn-around time for(i=0;i<n;i++)
{
ta=ta+bt[i];
...
printf("AVERAGE WAITING TIME : %f\n",wavg); printf("AVERAGE TURN AROUND TIME : %f\n",tavg);
 return 0;
}
